...
import tree from '@/pages/user/trainingCenter/components/tree.vue'
...
onMounted(() => {
    ...
    //         }
    //     })
    //     const getStaff = (EmpCode: string) => {
    //         return simplified.filter((items: any) => items.Manager === EmpCode).map((emp: any) => {
    //             emp.children = getStaff(emp.EmpCode)
    //             return emp
    //         })
    //     }

    //     newItems.value = simplified.filter((emp: any) => emp.EmpCode === 'RDMC0068').map((emp: any) => {
    //         emp.children = getStaff(emp.EmpCode)
    //         return emp
    //     })
...
// watch(empsRef, (newValue: any) => {
//     const ceo = 'RDMC0187'
//     newItems.value = newValue

//     // newItems.value = newValue.filter((item: any) => item.EmpCode === ceo)[0]
// })
</script>
