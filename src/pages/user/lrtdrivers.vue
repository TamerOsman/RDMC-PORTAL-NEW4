...
import { child, ref as fbRef, get, getDatabase, push, set } from 'firebase/database'
import { useAppStore } from '@/stores/app'
import * as XLSX from 'xlsx'
// import item from '@/pages/user/components/dutytable/item.vue'
import { ClockPauseIcon, FingerprintIcon, MugIcon, PaperclipIcon } from 'vue-tabler-icons'
...
function downloadPDF() {
    ...
    // Configure and generate PDF in landscape mode
    const opt = {
        margin: 1,
        filename: 'my-landscape-pdf-file.pdf',
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'in', format: 'letter', orientation: 'landscape' }, // Set to 'landscape'
    }
...
onMounted(() => {
    ...
    //         const consolidatedByDuty = r1.reduce((acc, item) => {
    //             if (!acc[item.duty]) {
    //                 acc[item.duty] = {
    //                     duty: item.duty,
    //                     activities: [],
...
//                     // dutytable.value = dkeys.map((key: string | number) => {
...
async function getCountOfEntries() {
    const lrtDutyTableRef = fbRef(db, '/operations/lrt_duty_table/')
    const snapshot = await get(lrtDutyTableRef)

    if (snapshot.exists()) {
        return snapshot.size + 1 // Returns the number of children in the ref
    } else {
        return 1 // If no entries, return 0
    }
}
    ...
    if (file) {
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
            ...
            if (binaryStr) {
                ...
                let dataRef = XLSX.utils.sheet_to_json(firstSheet)
                const creationDate = getCurrentDateTime()

                set(fbRef(db, `/operations/lrt_duty_table/${newKey}/`), {
                    creationDate, fileName, createdBy: appStore.empId, status: true,
                })
                // eslint-disable-next-line prefer-const
                // let result: Record<string, Item[]> = {}
                dataRef = dataRef.map(item => {
                    // result[item.duty] = item
                    const newRef = push(fbRef(db, `/operations/lrt_duty_table/${newKey}/data/`))
                    set(newRef, item)
                    return item
                })
...
<template>
    ...
    <v-layout class="fill-height w-screen d-flex flex-column overflow-auto">
    ...
    {{ dutytable }}
        ...
        <div v-if="!loading" ref="pdfContent" class="content-to-export">
            <v-container>
